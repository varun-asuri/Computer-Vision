...
void part2() {
    srand(time(NULL));
    ifstream read("hard.ppm");
    ofstream ppm1("imagef.ppm");
    ofstream ppm2("coins.ppm");
    ...
    int d = 0, n = 0, p = 0, q = 0, s = 0, random;
    for(int z = 0; z < width*height; z++) {
        if(votesct[z] > 200) {
            random = rand() % 5;
            if(random == 0) {
    ...
    q = q/30;
    s = s/30;
    debug << p << " pennies, " << n << " nickels, " << d << " dimes, " << q << " quarters, " << s << " silver dollars." << endl;
    int cost = p + (5*n) + (10*n) + (25*n) + (50*s);
    debug << "Total sum: $" << cost/100 << "." << cost%100;
